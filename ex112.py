# Exercício Python 112: Dentro do pacote utilidadesCeV que criamos
# no desafio 111, temos um módulo chamado dado. Crie uma função chamada
# leiaDinheiro() que seja capaz de funcionar como a função imputa(),
# mas com uma validação de dados para aceitar apenas valores que seja
# monetários.


from exmodulos import resumo, leiaDinheiro

valor = input('digite o preço R$: ')
valor = leiaDinheiro(valor)
aumento = int(input('defina a taxa de aumento: '))
reducao = int(input('defina a taxa de redução: '))

resumo(valor, aumento, reducao)
